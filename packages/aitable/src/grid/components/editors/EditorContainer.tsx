...
const NO_EDITING_CELL_TYPES = new Set([
  CellType.Boolean,
  CellType.Rating,
  CellType.Button,
]);
...
> = (props, ref) => {
  ...
  const height =
    activeCellBound?.height ?? coordInstance.getRowHeight(rowIndex);
  ...
  useEffect(() => {
    if ((cellContent as ICell).type === CellType.Loading) {
      return;
    }
  ...
  useEffect(() => {
    if ((cellType as CellType) === CellType.Loading) {
      return;
    }
  ...
  const rect = useMemo(() => {
    const { rowInitSize, columnInitSize, containerWidth, containerHeight } =
      coordInstance;
    const x = clamp(
