...
import {
  VirtualFieldConfig,
  type IVirtualFieldConfigRef,
  type IFormulaFieldConfigOptions,
  type IRollupFieldConfigOptions,
} from "./VirtualFieldConfig";
...
> = ({ onConfirm, onCancel }, ref) => {
  ...
  const [selectedType, setSelectedType] =
    useState<IFieldTypeModal>("singleLineText");
  const [fieldName, setFieldName] = useState("");
  const [isCreating, setIsCreating] = useState(false);
  const [isConfiguring, setIsConfiguring] = useState(false);
  const [fieldOptions, setFieldOptions] = useState<
    IFormulaFieldConfigOptions | IRollupFieldConfigOptions | undefined
  >();
  ...
  const [numberDisplay, setNumberDisplay] = useState<"number" | "ring" | "bar">(
    "number",
  );
  ...
  const handleVirtualFieldConfigConfirm = (
    options: IFormulaFieldConfigOptions | IRollupFieldConfigOptions,
  ) => {
    ...
    if (fieldName.trim()) {
      ...
      if (selectedType === "number") {
        options = {
          ...
          defaultValue:
            numberDefaultValue !== "" ? Number(numberDefaultValue) : undefined,
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNumberPrecision(parseInt(e.target.value) || 0)
                    }
                    style={{
                      width: "120px",
                      padding: "8px 12px",
