...
import {
  RowContextMenu,
  type IRowContextMenuRef,
} from "../components/context-menu/RowContextMenu";
import {
  DeleteConfirmDialog,
...
const GridBase: ForwardRefRenderFunction<IGridRef, IGridProps> = (
  props,
  forwardRef,
) => {
  const {
  ...
  const [mouseState, setMouseState] =
    useState<IMouseState>(DEFAULT_MOUSE_STATE);
  ...
  const containerHeight = hasColumnStatistics
  ...
  const columnInitSize = useMemo(() => {
    return !rowIndexVisible && !rowControlCount
      ? 0
  ...
  const defaultRowsInfo = useMemo(() => {
    return {
      ...
      rowHeightMap: hasAppendRow
        ? { [originRowCount]: appendRowHeight }
        : undefined,
    };
  ...
  const getLinearRow = useCallback(
    (index: number): ILinearRow => {
      ...
      return (
        linearRows[index] ??
        ({ type: LinearRowType.Row, realIndex: -2 } as ILinearRow)
      );
    },
  ...
  const activeCellBound = useMemo(() => {
    ...
    const originHeight =
      coordInstance.getRowHeight(real2RowIndex(activeRowIndex)) ?? 0;
  ...
  // 处理编辑字段确认
  const handleEditFieldConfirm = useCallback(
    (payload: { type: string; name: string; options?: any }) => {
  ...
  // 处理行号右键菜单
  const handleRowHeaderMenuClick = useCallback(
    (rowIndex: number, position: IPosition) => {
  ...
  const handleCellContextMenu = useCallback(
    (rowIndex: number, _colIndex: number, position: IPosition) => {
  ...
  const openEditFieldAtColumn = useCallback(
    (colIndex: number) => {
      const containerEl = containerRef.current;
      const rect = containerEl?.getBoundingClientRect();
      ...
      <EditFieldMenu
        ...
        initialData={
          editingColumnIndex >= 0
            ? {
                type: mapAPITypeToMenuType(
                  (columns[editingColumnIndex] as any)?.type || "text",
                ),
                name: columns[editingColumnIndex]?.name || "",
                options: (columns[editingColumnIndex] as any)?.options || {},
                description:
                  (columns[editingColumnIndex] as any)?.description || "",
              }
            : undefined
        }
      />
