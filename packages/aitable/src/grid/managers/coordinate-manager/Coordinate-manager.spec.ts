...
describe("CoordinateManager", () => {
  ...
  test("findNearestCellIndex", () => {
    ...
    expect(coordinateManager.findNearestCellIndex(300, ItemType.Column)).toBe(
      4,
    );
  });

  test("getCellMetaData", () => {
    const rowMetaData = (coordinateManager as any).getCellMetaData(
      2,
      ItemType.Row,
    );
    const columnMetaData = (coordinateManager as any).getCellMetaData(
      2,
      ItemType.Column,
    );
