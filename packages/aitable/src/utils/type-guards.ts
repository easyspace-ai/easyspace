...
export function isPrimitive(
  ...
  return (
    isString(value) || isNumber(value) || isBoolean(value) || isNullish(value)
  );
}
...
export function isViewType(value: unknown): value is string {
  return (
    isString(value) && ["grid", "kanban", "calendar", "gallery"].includes(value)
  );
}
...
export function safeArrayFind<T>(
  ...
  return array.find((item, index) => predicate(item as T, index)) as
    | T
    | undefined;
}
