...
const useValidators = ({
  ...
  const onFilesSuccessfullySelected = react.useCallback(
    (data) => {
  ...
  const onFilesRejected = react.useCallback(
    (errors) => {
...
function useFilePicker(props = {}) {
  ...
  const [plainFiles, setPlainFiles] = react.useState([]);
  ...
  const clearWithEventListener = react.useCallback(() => {
    clear();
  ...
  const openFilePicker = react.useCallback(() => {
    ...
    openFileDialog(
      ...
      (evt) => __async(null, null, function* () {
        clear();
        setLoading(true);
        const plainFileObjects = yield fileSelector.fromEvent(evt);
...
function useImperativeFilePicker(props) {
  ...
  const [allPlainFiles, setAllPlainFiles] = react.useState([]);
  const [allFilesContent, setAllFilesContent] = react.useState([]);
  ...
  const removeFileByReference = react.useCallback(
    (file) => {
